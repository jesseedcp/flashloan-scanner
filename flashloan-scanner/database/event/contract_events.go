...
type ContractEventsView interface {
	...
	ChainContractEventsByTxHash(string, common.Hash) ([]ContractEvent, error)
...
func (db *contractEventsDB) ChainContractEventsByTxHash(chainId string, txHash common.Hash) ([]ContractEvent, error) {
	var events []ContractEvent
	result := db.gorm.Table("contract_events_"+chainId).
		Where("transaction_hash = ?", txHash.Hex()).
