...
type UniswapV2RunnerConfig struct {
	ScannerName  string
...
func BuildUniswapV2Runner(
	db *database.DB,
	rawClient *rpc.Client,
	chainID uint64,
	cfg UniswapV2RunnerConfig,
	...
	if cfg.ScannerName == "" {
		cfg.ScannerName = "uniswap_v2_scanner"
	}
	...
	extractor, err := scannerextractor.NewUniswapV2CandidateExtractor(reg)
	if err != nil {
		return nil, err
	}
	verifier, err := scannerverifier.NewUniswapV2Verifier(db.ContractEvents, db.ObservedTx, db.InteractionAssetLeg)
	if err != nil {
		return nil, err
	}
	var interactionVerifier scannerverifier.InteractionVerifier = verifier
	if cfg.TraceEnabled {
		traceVerifier, err := scannerverifier.NewUniswapV2TraceVerifier(verifier, scannertrace.NewGethProvider(rawClient))
		if err != nil {
			return nil, err
		}
